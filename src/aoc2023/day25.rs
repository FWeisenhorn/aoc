...
fn part_a(input: &str) -> String {
    ...
    let end = furthest(&data, start);
    // Find the size of the graph still connected to start after the cut.
    let size = flow(&data, start, end);
    (size * (data.nodes.len() - size)).to_string()
}
...
mod tests {
    ...
    const _TEST: &str = include_str!("tests/day25.txt");

    #[test]
    fn test() {
        assert_eq!(part_a(_TEST), "54");
    }
}
